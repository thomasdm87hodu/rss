...
function enumCaseId(field, name) {
  if (field.type !== "enum") return name;
  const enumCase = field.cases?.find((candidate) => candidate.name === name);
  if (!enumCase) throw new Error(`Framer field “${field.name}” is missing the “${name}” option`);
  return enumCase.id;
}
...
function slugify(value) {
  return value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 80) || "daily-news-update";
}

function uniqueSlug(title, date, items, currentItemId) {
  const usedSlugs = new Set(items.filter((item) => item.id !== currentItemId).map((item) => item.slug));
  const base = slugify(title);
  if (!usedSlugs.has(base)) return base;

  const suffix = `-${date}`;
  const dated = `${base.slice(0, 80 - suffix.length).replace(/-+$/, "")}${suffix}`;
  if (!usedSlugs.has(dated)) return dated;

  for (let attempt = 2; attempt < 100; attempt += 1) {
    const counter = `-${attempt}`;
    const candidate = `${dated.slice(0, 80 - counter.length).replace(/-+$/, "")}${counter}`;
    if (!usedSlugs.has(candidate)) return candidate;
  }

  throw new Error("Unable to create a unique Framer slug");
}

function articleNumberValue(value) {
  const match = String(value || "").match(/(\d+)\s*$/);
  return match ? Number(match[1]) : null;
}

function highestArticleNumber(items, numberField) {
  let highest = 0;
  for (const item of items) {
    const number = articleNumberValue(item.fieldData?.[numberField.id]?.value);
...
function nextArticleNumber(items, numberField) {
  return formatArticleNumber(highestArticleNumber(items, numberField) + 1);
}

async function getCategoryId(collections) {
...
function makeFieldData({ fields, article, date, number, categoryId }) {
  const data = {};
  ...
  data[fields.author.id] = fieldEntry(fields.author, enumCaseId(fields.author, AUTHOR));
  data[fields.date.id] = fieldEntry(fields.date, `${date}T00:00:00.000Z`);
  data[fields.category.id] = fieldEntry(fields.category, categoryId);
  data[fields.featured.id] = fieldEntry(fields.featured, true);
  ...
  if (fields.propheticConnection) {
    data[fields.propheticConnection.id] = fieldEntry(
      fields.propheticConnection,
      enumCaseId(fields.propheticConnection, "None"),
    );
  }
...
export async function saveFramerDraft({
  ...
  const categoryId = await getCategoryId(collections);
  ...
  // Preserve a corrected number on the existing daily draft. Replace only the
  // initial [No. 0001] placeholder when earlier numbered posts already exist.
  const otherItems = items.filter((item) => item.id !== draftForDate?.id);
  const highestOtherNumber = highestArticleNumber(otherItems, fields.number);
  const currentDraftNumber = articleNumberValue(draftForDate?.fieldData?.[fields.number.id]?.value);
  const preserveCurrentNumber = draftForDate
    && currentDraftNumber !== null
    && currentDraftNumber >= highestOtherNumber
    && !(currentDraftNumber === 1 && highestOtherNumber >= 1);
  const number = preserveCurrentNumber
    ? formatArticleNumber(currentDraftNumber)
    : nextArticleNumber(otherItems, fields.number);
  const fieldData = makeFieldData({ fields, article, date, number, categoryId });
  const slug = uniqueSlug(article.title, date, items, draftForDate?.id);

  if (draftForDate) {
    await draftForDate.setAttributes({ draft: true, slug, fieldData });
    return { item: { id: draftForDate.id, slug }, number, created: false, updated: true };
  }
