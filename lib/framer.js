...
export async function saveFramerDraft({
  ...
  // Recalculate from published items plus other drafts so a first failed/test
  // draft cannot leave the daily sequence stuck at its initial placeholder.
  const number = nextArticleNumber(
    items.filter((item) => item.id !== draftForDate?.id),
    fields.number,
  );
