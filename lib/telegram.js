...
export const DEFAULT_CHANNEL = "breakingisraelnews";
...
export function parseTelegramPreview(html, pageUrl = getTelegramPreviewUrl()) {
  ...
  $(".tgme_widget_message").each((_, element) => {
    ...
    // Replies and forwarded blocks are not part of the channel post itself.
    content
      .find(
        ".tgme_widget_message_reply, .tgme_widget_message_forwarded_from, .tgme_widget_message_forwarded_from_name, .tgme_widget_message_forwarded_from_author",
      )
      .remove();
